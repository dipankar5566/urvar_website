    ...
    <section className="bg-white py-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          {/* Connector line */}
          <div className="hidden lg:block absolute top-8 left-0 right-0 h-0.5 bg-gradient-to-r from-urvar-green via-green-400 to-urvar-green" />

          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-6">
            {stages.map((stage, i) => (
              <div key={i} className="flex flex-col items-center text-center relative">
                {/* Circle */}
                <div className="relative z-10 w-16 h-16 rounded-full bg-urvar-dark flex items-center justify-center mb-4 shadow-lg ring-4 ring-white">
                  <span className="text-2xl">{stage.emoji}</span>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 w-full">
                  <span className="text-xs font-bold text-urvar-green uppercase tracking-wide mb-1 block">
                    Stage {stage.number}
                  </span>
